...
## 数据预处理
&emsp;&emsp;基本想法是遍历训练集，根据网约车gps的信息、运动方向和各路口的位置统计出各时间段经过各路口的车辆数，统计出各时间段经过各路口的车辆数. 由于训练集中已有路过车辆的平均速度，因此暂时不考虑统计网约车的速度情况，若有需要后期再进行调整. 网约车gps数据的清理在 /data_cleaning 中.